# e04s01 — Spectrum engine: FFT worker + 32 normalized bars in a drawlist
...
and enqueues a main-thread redraw of 32 bars (drawlist rectangles). The math is pure and
unit-testable: `compute_spectrum_bars(samples) -> np.ndarray` (Hann window, rfft, dB scale
with -60 dBFS floor, 32 equal bins) and the bars are drawn via `ui_task` (HIGH-1).

## Requirements

#### ENHANCED: Live spectrum bars when level analysis is on
**Before:** only a VU progress bar.
**After:** a 330x90 drawlist with 32 vertical bars (green), each 0..1 level from the FFT;
...
1. Pure `compute_spectrum_bars(samples) -> np.ndarray` (32 bins, 0..1, dB floor) + unit
